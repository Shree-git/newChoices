...
export class ImpulseService {
  ...
  getAllImpulses(userId: string): Observable<Impulse[]>{
    // console.log(this.aService.getUser().email)
    this.userId = userId
    this.impulsesCollection = this.afStore.collection('users').doc(userId).collection('impulses')
  ...
  searchImpulse(userId, searchWord){
    // searchWord = searchWord.toUpperCase()
    searchWord = Number(searchWord)
    return this.afStore.collection('users').doc(userId).collection('impulses' , ref => 
  ...
  sortBy(rating: string, desAsc){
    return this.afStore.collection('users').doc(this.userId).collection('impulses' , ref => 
